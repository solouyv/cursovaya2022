...
from exchange.api.accountant.forms import AddCurrencyRateForm, ShowCurrencyRatesForm, GetMonthlyCompensation, MonthlyReport
...
@inject
async def get_form(request: Request, session_data: SessionData = Depends(verifier)):
...
@inject
async def add_data(request: Request, session_data: SessionData = Depends(verifier)):
...
@inject
async def show_currency_rates(request: Request, session_data: SessionData = Depends(verifier)):
...
@inject
async def get_form(request: Request, session_data: SessionData = Depends(verifier)):
    form = MonthlyReport(request)
    form.get()
    form.__dict__.update(session_data.user)
    return templates.TemplateResponse("accountant/monthly_report.html", form.__dict__)


@router.get("/accountant/get-monthly-compensation", response_class=HTMLResponse, dependencies=[Depends(cookie)])
@inject
async def show_currency_rates(request: Request, session_data: SessionData = Depends(verifier)):
    form = GetMonthlyCompensation(request)
    form.get_currency_rates()
    form.__dict__.update(session_data.user)
    return templates.TemplateResponse("accountant/month_compensation.html", form.__dict__)
